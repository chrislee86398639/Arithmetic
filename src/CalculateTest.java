...
private static Calculate calculate =new Calculate();
	...
	/*outcome其实实际就是ToSuffixExpression + testCalculate
	 * @Test void testOutcome() { //字符串为空情况 String s1=""; calculate.outcome(s1); }
	 */

	@Test
	void testToSuffixExpression() throws IOException {
		
        String sMid ="3 * 2 + 2 / 1 - 2 + 1 =";
	...
	void testCalculate() throws IOException {
        ...
        String strINPUT ="3 * 2 + 2 / 1 - 2 + 1 = ";//测试用例
        System.out.println("计算表达式："+strINPUT);
        
        String Excepted="7/1";//预计结果，string形式
